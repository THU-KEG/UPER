...
from s2s_ft.modeling import BertForSequenceToSequence, BertForSequenceToSequence_Distill, \
...
def prepare_for_training(args, model, checkpoint_state_dict, amp):
    ...
    if (args.use_distill > 0):
        ...
        teacher_model_pt = OrderedDict(
            (k[5:] if k.startswith('bert') else k, v) for k, v in teacher_pt.items() if k.startswith('bert'))
        teacher_cls_pt = OrderedDict(
            (k[4:] if k.startswith('cls') else k, v) for k, v in teacher_pt.items() if k.startswith('cls'))
...
def train(args, training_features, model, tokenizer):
    ...
    train_dataset = utils.Seq2seqDatasetForBert(
        ...
        word_drop_prob=args.word_drop_prob, word_shuffle_k=args.word_shuffle_k, sent_shuffle_k=args.sent_shuffle_k,
        sent_drop_prob=args.sent_drop_prob
    )

    logger.info("Check dataset:")
    for i in range(5):
        source_ids, noisy_source_ids_s, noisy_source_ids_t, target_ids, pseudo_ids, num_source_tokens, noisy_num_source_tokens_s, noisy_num_source_tokens_t, num_target_tokens = train_dataset.__getitem__(
            i)
