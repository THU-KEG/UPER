...
class BertPreTrainedForSeq2SeqModel(BertPreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, reuse_position_embedding=None, *model_args, **kwargs):
        ...
        try:
            ...
            print(kwargs)
            print("#" * 26)
            logger.info(f"No args state_dict, {kwargs}")
...
class BertForSequenceToSequence(BertPreTrainedForSeq2SeqModel):
    ...
    def forward(self, source_ids, noisy_source_ids, target_ids, pseudo_ids, num_source_tokens, num_noisy_source_tokens,
                num_target_tokens, target_span_ids=None):
...
class BertForSequenceToSequence_Distill(BertPreTrainedForSeq2SeqModel):
    ...
    def forward(self, source_ids, noisy_source_ids_s, noisy_source_ids_t, target_ids, pseudo_ids, num_source_tokens,
                num_noisy_source_tokens_s, num_noisy_source_tokens_t, num_target_tokens, target_span_ids=None):
        if (self.use_noisy_student > 0):
            if (self.use_random_replace_input > 0):
                with torch.no_grad():
                    ...
                    noisy_words = torch.masked_select(top_words,
                                                      torch.tensor(indices).to(torch.bool).to(
                                                          noisy_source_ids_s.device)).reshape(
                        top_words.shape[0], top_words.shape[1])
                    indices = torch.bernoulli(
                        torch.ones(*noisy_words.shape) * (1 - self.random_replace_input_p)).long().to(
        ...
        if (self.use_noisy_teacher > 0):
            if (self.use_random_replace_input > 0):
                with torch.no_grad():
                    ...
                    noisy_words = torch.masked_select(top_words,
                                                      torch.tensor(indices).to(torch.bool).to(
                                                          noisy_source_ids_t.device)).reshape(
                        top_words.shape[0], top_words.shape[1])
                    indices = torch.bernoulli(
                        torch.ones(*noisy_words.shape) * (1 - self.random_replace_input_p)).long().to(
        ...
        student_position_ids = torch.cat(
            (student_source_position_ids, student_target_position_ids, student_target_position_ids), dim=1)
        teacher_position_ids = torch.cat(
            (teacher_source_position_ids, teacher_target_position_ids, teacher_target_position_ids), dim=1)
        ...
        student_attention_mask = self.create_attention_mask(student_source_mask, student_target_mask,
                                                            student_source_position_ids, student_target_span_ids)
        teacher_attention_mask = self.create_attention_mask(teacher_source_mask, teacher_target_mask,
                                                            teacher_source_position_ids, teacher_target_span_ids)
        ...
        kd_loss = self.get_kd_loss(F.log_softmax(student_prediction_scores_masked.float(), -1),
                                   F.softmax(teacher_prediction_scores_masked.float(), -1))
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
